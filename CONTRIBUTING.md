...
向本仓库提交内容，表示你确认自己有权提交这些材料，并同意按仓库的双协议授权：

- 提交到 `content/`、`inbox/`、`docs/`、`templates/` 等位置的原创知识库文字内容，默认按 Creative Commons Attribution-ShareAlike 4.0 International 授权。
- 提交的代码、构建脚本、配置和本地插件修改，默认按 MIT License 授权。
- 转载、引用、截图、头像、商标和其他第三方材料必须注明来源和原始授权；没有明确授权的材料不要提交。
