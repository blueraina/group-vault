...
var explicitMathPattern = /(\$\$[\s\S]+?\$\$|\$[^$\n]+?\$|\\\([\s\S]+?\\\)|\\\[[\s\S]+?\\\])/g;
var groupedImplicitMathPattern = /(?:[A-Za-z][A-Za-z0-9]*(?:\^\{[^}]+\})?)?\([^()\n]*\\[A-Za-z][^()\n]*\)(?:\s*\\[A-Za-z]+\s*(?:[A-Za-z][A-Za-z0-9]*(?:\^\{[^}]+\})?)?\([^()\n]*\\[A-Za-z][^()\n]*\))*/g;
var commandImplicitMathPattern = /\\[A-Za-z]+(?:\s*(?:[_^=+*/<>#{}\\A-Za-z0-9]|[()[\],.]))*/g;
...
function renderMath(math, key) {
  try {
    return /* @__PURE__ */ u2("span", { key, class: "toc-math", dangerouslySetInnerHTML: {
...
function collectImplicitMathRanges(text) {
  const ranges = [];
  ...
  return ranges.sort((a, b) => a[0] - b[0] || b[1] - a[1]).reduce((merged, range) => {
...
function renderImplicitMath(text, prefix) {
...
function renderTocText(text) {
  ...
  for (const match of text.matchAll(explicitMathPattern)) {
    ...
    parts.push(renderMath(stripMathDelimiters(match[0]), `toc-explicit-${explicitIndex}`));
      ...
      /* @__PURE__ */ u2(
        OverflowList2,
        {
          ...
          children: fileData.toc.map((tocEntry) => {
            ...
            return /* @__PURE__ */ u2("li", { class: `depth-${depth}`, children: /* @__PURE__ */ u2("a", { href: `#${slug2}`, "data-for": slug2, children: renderTocText(text) }) }, slug2);
          })
        }
      )
    ] });
