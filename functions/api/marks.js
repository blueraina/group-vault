...
async function handleGet(request, env) {
  ...
  if (pathParam) {
    ...
    const { results } = await db
      .prepare(
        `SELECT path, mark_type, created_at, updated_at
         FROM user_note_marks
         WHERE github_login = ? AND path = ?
         ORDER BY updated_at DESC`,
      )
      .bind(user.login, path)
      .all()
...
async function handlePut(request, env) {
  ...
  if (!active) {
    await db
      .prepare("DELETE FROM user_note_marks WHERE github_login = ? AND path = ? AND mark_type = ?")
      .bind(user.login, path, markType)
      .run()
    return json({ ok: true, mark: { path, mark_type: markType, active: false } })
  }

  await db
    .prepare(
      `INSERT INTO user_note_marks (github_login, path, mark_type, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(github_login, path, mark_type)
       DO UPDATE SET updated_at = excluded.updated_at`,
    )
    .bind(user.login, path, markType, now, now)
    .run()

  return json({ ok: true, mark: { path, mark_type: markType, active: true, updated_at: now } })
}
