...
- 支持已读、收藏标记，并在图谱中用颜色区分。
- 支持 GitHub 登录，已读、收藏、评论均与 GitHub 账号绑定。
- 提供评论管理页，方便查看和处理近期评论
...
AI 找笔记按 GitHub 登录用户限流：每分钟最多 10 次，并通过 `COMMENTS_DB` 记录每天最多 200 次的持久额度。每日额度在日本时间 00:00 重置；部署包含新迁移的版本前，必须先应用 D1 迁移。
